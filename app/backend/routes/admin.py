...
def email_notifications():
    ...
    email_form.os_type.data = current_app.config.get('OS_TYPE', 'auto')
...
def update_email_config():
    ...
    if form.validate_on_submit():
        ...
        # Update OS type and email settings
        env_vars['OS_TYPE'] = form.os_type.data
        ...
        with open(env_path, 'w') as f:
            ...
            f.write('# Operating System: auto, windows, macos, linux\n')
            for key, value in env_vars.items():
                f.write(f'{key}={value}\n')
        
        # Update current app config
        current_app.config['OS_TYPE'] = form.os_type.data
